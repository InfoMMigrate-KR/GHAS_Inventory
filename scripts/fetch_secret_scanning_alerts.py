...
@retry_on_failure()
def fetch_user_email(username: str, pat_cycler: itertools.cycle, repo_full_name: str = None) -> Optional[str]:
    """
    ...
    try:
        ...
        if not email and repo_full_name:
            try:
                ...
                commits_response = requests.get(commits_url, headers=headers, params=params, timeout=TIMEOUT)
                ...
                for commit in commits:
                    ...
                    if commit_email and "@users.noreply.github.com" not in commit_email:
                        logging.debug(f"Found email for {username} from commit history: {commit_email}")
                        return commit_email
                    elif commit_email:
                        # Save noreply email as fallback
                        email = commit_email
            except Exception as commit_error:
                logging.debug(f"Failed to fetch email from commits for {username}: {commit_error}")
...
@retry_on_failure()
def fetch_repo_admins(repo_full_name: str, pat_cycler: itertools.cycle) -> Dict[str, str]:
    """
    ...
    try:
        ...
        for collab in collaborators:
            if collab.get("login"):
                ...
                email = fetch_user_email(collab.get("login"), pat_cycler, repo_full_name)
...
def enrich_secret_data_with_commit_info(
    ...
    for alert in secret_data:
        try:
            ...
            if org_name and repo_name:
                repo_full_name = f"{org_name}/{repo_name}"
                admin_data = repo_admins_cache.get(repo_full_name, {"logins": None, "emails": None})
